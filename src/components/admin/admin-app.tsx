...
function MediaPage() {
  ...
  async function load() { const filter = selectedFolder ? `&where[folder][equals]=${encodeURIComponent(selectedFolder)}` : ''; const [mediaData, folderData] = await Promise.all([api(`/media?limit=60&sort=-createdAt${filter}`), api('/globals/media-folders').catch(() => ({ folders: [] }))]); setMedia(mediaData); setFolders((folderData.folders || []).map((folder: { name: string }) => folder.name).filter(Boolean)) }
