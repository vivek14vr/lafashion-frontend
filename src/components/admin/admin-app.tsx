...
function GalleryForm() {
  ...
  const field = (name: 'title' | 'date' | 'location' | 'excerpt', label: string, required = false) => <label className="admin-form"><span>{label}</span><input required={required} value={data[name]} onChange={(event) => setData((current) => ({ ...current, [name]: event.target.value }))} /></label>
  return <form className="admin-form admin-gallery-form" onSubmit={save}><div className="admin-page-heading"><div><p className="admin-eyebrow">WEBSITE</p><h1>Create gallery</h1><p className="admin-muted">Create a photo archive for a past fashion show.</p></div><button className="admin-primary" disabled={saving || !data.images.length || (data.source === 'platform' && !data.event)}>{saving ? 'Creating…' : 'Create gallery'}</button></div>{error ? <p className="admin-error">{error}</p> : null}<div className="admin-gallery-details">{field('title', 'Gallery title', true)}<label className="admin-form"><span>Gallery type</span><select value={data.source} onChange={(event) => setData((current) => ({ ...current, source: event.target.value, event: event.target.value === 'platform' ? current.event : '' }))}><option value="standalone">Standalone past show</option><option value="platform">Linked to a platform event</option></select></label>{data.source === 'platform' ? <label className="admin-form"><span>Link to past event</span><select required value={data.event} onChange={(event) => setData((current) => ({ ...current, event: event.target.value }))}><option value="">Choose a past event</option>{events.map((event) => <option value={event.id} key={event.id}>{String(event.title || event.id)}{event.date ? ` — ${new Date(String(event.date)).toLocaleDateString()}` : ''}</option>)}</select>{events.length === 0 ? <small className="admin-muted">No events are available yet. Create the event first.</small> : null}</label> : null}{field('date', 'Show date')}{field('location', 'Location')}{field('excerpt', 'Short blurb')}</div><MediaChoice label="Cover image (optional)" value={data.coverImage} media={media} onChange={(value) => setData((current) => ({ ...current, coverImage: value }))} /><section className="admin-gallery-picker"><div className="admin-gallery-picker__heading"><div><h2>Gallery photos</h2><p className="admin-muted">Choose images from one folder. Selected: {data.images.length}</p></div>{pickerFolder ? <button type="button" className="admin-secondary" onClick={() => setPickerFolder('')}>← Back to folders</button> : null}</div>{pickerFolder === '' ? <div className="admin-picker-folders">{folderCards.map((folder) => <button type="button" className="admin-picker-folder" key={folder.id} onClick={() => setPickerFolder(folder.id)}><FolderOpen size={20} /><strong>{folder.name}</strong><small>{media.filter((image) => folder.id === '__unsorted__' ? !image.folder : image.folder === folder.id).length} images</small></button>)}</div> : visibleMedia.length ? <div className="admin-destination-picker__grid">{visibleMedia.map((image) => { const selected = data.images.includes(image.id); return <label className={`admin-destination-image ${selected ? 'is-selected' : ''}`} key={image.id}><input type="checkbox" checked={selected} onChange={() => toggleImage(image.id)} /><span className="admin-destination-image__preview">{image.url ? <img src={String(image.url)} alt={String(image.filename || '')} /> : null}</span><small>{String(image.filename || image.id)}</small></label> })}</div> : <p className="admin-muted">No images in this folder yet.</p>}</section><div className="admin-form-actions"><button className="admin-primary" disabled={saving || !data.images.length || (data.source === 'platform' && !data.event)}>{saving ? 'Creating…' : 'Create gallery'}</button></div></form>
}
...
function MediaChoice({ label, value, media, onChange, required = false }: { label: string; value: string; media: Doc[]; onChange: (value: string) => void; required?: boolean }) {
  ...
  const folderNames = Array.from(new Set(media.map((image) => String(image.folder || '')).filter(Boolean))).sort((a, b) => a.localeCompare(b))
  const folderCards = [{ id: '__unsorted__', name: 'Unsorted' }, ...folderNames.map((name) => ({ id: name, name }))]
  const visibleMedia = media.filter((image) => folder === '__unsorted__' ? !image.folder : image.folder === folder)
  function clearSelection() { onChange(''); setFolder('') }
  return <div className="admin-form admin-media-choice"><span>{label}{required ? ' *' : ''}</span>{selected ? <div className="admin-media-choice__selected"><div className="admin-media-choice__selected-image">{selected.url ? <img src={String(selected.url)} alt={String(selected.filename || '')} /> : null}</div><div className="admin-media-choice__selected-copy"><strong>{String(selected.filename || selected.id)}</strong><small>{String(selected.folder || 'Unsorted')}</small></div><button type="button" className="admin-secondary" onClick={clearSelection}>Change image</button></div> : <div className="admin-media-choice__picker">{folder ? <div className="admin-media-choice__toolbar"><button type="button" className="admin-secondary" onClick={() => setFolder('')}>← Back to folders</button><small>{visibleMedia.length} images</small></div> : <p className="admin-media-choice__hint">Choose a folder first, then select an image.</p>}{folder === '' ? <div className="admin-picker-folders">{folderCards.map((item) => <button type="button" className="admin-picker-folder" key={item.id} onClick={() => setFolder(item.id)}><FolderOpen size={20} /><strong>{item.name}</strong><small>{media.filter((image) => item.id === '__unsorted__' ? !image.folder : image.folder === item.id).length} images</small></button>)}</div> : visibleMedia.length ? <div className="admin-destination-picker__grid">{visibleMedia.map((image) => <button type="button" className="admin-destination-image" key={image.id} onClick={() => onChange(image.id)}><span className="admin-destination-image__preview">{image.url ? <img src={String(image.url)} alt={String(image.filename || '')} /> : null}</span><small>{String(image.filename || image.id)}</small></button>)}</div> : <p className="admin-muted">No images in this folder yet.</p>}</div>}{required ? <input className="admin-media-choice__required" tabIndex={-1} required value={value} onChange={() => undefined} aria-label={label} /> : null}</div>
}
