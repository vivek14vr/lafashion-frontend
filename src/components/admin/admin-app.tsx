...
function GalleryForm() {
  ...
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  useEffect(() => { void Promise.all([api('/media?limit=200&sort=-createdAt'), api('/globals/media-folders').catch(() => ({ folders: [] }))]).then(([mediaData, folderData]) => { setMedia(mediaData.docs || []); setFolders((folderData.folders || []).map((folder: { name: string }) => folder.name).filter(Boolean)) }).catch(() => undefined) }, [])
  async function save(event: React.FormEvent) {
    event.preventDefault(); setSaving(true); setError(''); const stopLoading = startLoading('Creating gallery…')
    try { await api('/galleries', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...data, published: false }) }); router.replace('/admin/galleries') } catch (err) { setError(err instanceof Error ? err.message : 'Could not create gallery.') } finally { setSaving(false); stopLoading() }
  }
